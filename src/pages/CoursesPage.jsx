      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {courses.map((course) => (
            <div
              key={course.$id}
              className="relative rounded-lg bg-card p-6"
              style={{ borderTop: `5px solid ${course.colorCode}` }}
            >
              <div className="absolute top-4 right-4">
                <button onClick={() => toggleMenu(course.$id)} className="text-muted-foreground">
                  ...
                </button>
                {openMenuId === course.$id && (
                  <div className="absolute right-0 mt-2 w-32 rounded-md bg-background shadow-lg">
